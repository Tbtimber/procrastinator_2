...
public class ProcrastinatorDatabaseManager {
  ...
  /*  public static Movie movieFromCursor(Cursor c){ // crée un  curseur
        if (null != c){
            ...
            if (c.getColumnIndex(ProcrastinatorDatabaseContract.PLOT) >= 0){
                movie.plot = c.getString(c.getColumnIndex(ProcrastinatorDatabaseContract.PLOT));
            }

            if (c.getColumnIndex(ProcrastinatorDatabaseContract.TITLE) >= 0){
                movie.title = c.getString(c.getColumnIndex(ProcrastinatorDatabaseContract.TITLE));
            }

            if (c.getColumnIndex(ProcrastinatorDatabaseContract.MOVIE_PICTURE) >= 0){
                movie.moviePicture = c.getString(c.getColumnIndex(ProcrastinatorDatabaseContract.MOVIE_PICTURE));
            }
    ...
    /*public static ContentValues movieToContentValues(SearchResult movie){
        ...
        if (!TextUtils.isEmpty(movie.plot)){
            values.put(ProcrastinatorDatabaseContract.PLOT, movie.plot);
        }

        if (!TextUtils.isEmpty(movie.title)){
            values.put(ProcrastinatorDatabaseContract.TITLE, movie.title);
        }

        if (!TextUtils.isEmpty(movie.moviePicture)){
            values.put(ProcrastinatorDatabaseContract.MOVIE_PICTURE, movie.moviePicture);
        }
